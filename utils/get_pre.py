...
def get_pre(test_path, dict_id, dict_id_all, new_d_all, Feature_train, target_train, opt, model, device, th, it=4):
    ...
    with torch.no_grad():
        ...
        output = F.cosine_similarity(
            torch.mul(torch.ones(Feature_train.shape).to(device), feature_test[0, :].T.to(device)),
            Feature_train.to(device), dim=1).to(device)
        kind = torch.zeros(len(dict_id_all))
        ...
        top4_index = indices.cpu().detach().numpy()[0:it]
        top4 = sorted.cpu().detach().numpy()[0:it]
