...
def get_pre(test_path, dict_id, dict_id_all, new_d_all, Feature_train, target_train, opt, model, device, th, it=-4):
    ...
    with torch.no_grad():
        feature_test = model(test_tensor)
        for j in range(len(dict_id_all)):
            Feature_train_F = Feature_train[(target_train == j).reshape(-1), :]
            ...
            Score[j] = output.cpu().detach().numpy().mean()
        top4_index = Score.argsort()[it:0]
        top4 = Score.sort()[it:0]
        top4 = top4[top4 > th]
