...
def get_model(backbone, pretrained, num=512):
    if backbone == 'EfficientNet-V2':
        model = timm.create_model('efficientnetv2_rw_m', pretrained=pretrained, num_classes=num)
    elif backbone == 'resnet101':
        model = torchvision.models.resnet101(pretrained=pretrained)
        model.fc = torch.nn.Linear(model.fc.in_features, num)
        ...
        model = timm.create_model('convnext_tiny', pretrained=pretrained, num_classes=num)
    elif backbone == 'convnext_small':
        model = timm.create_model('convnext_small', pretrained=pretrained, num_classes=num)
    elif backbone == 'convnext_base':
        model = timm.create_model('convnext_base', pretrained=pretrained, num_classes=num)
    elif backbone == 'convnext_large':
        model = timm.create_model('convnext_large', pretrained=pretrained, num_classes=num)
    elif backbone == 'swin_base_patch4_window7_224':
        model = timm.create_model('swin_base_patch4_window7_224', pretrained=pretrained, num_classes=num)
    else:
