...
def go_predict(args):
    with torch.no_grad():
        ...
        model, dict_id, dict_id_species, Feature_train, target_train = get_pre_need(args.model_path,
                                                                                    args.dict_id_path,
                                                                                    args.dict_id_species_path,
        ...
        if args.Feature_test_path is None:
            ...
            target_test = np.load(args.target_test_path)

        if args.Top_all_path is None:
            # 获取各个总类中心点
            Feature_train_num = np.zeros([len(dict_id), 512])
            for item in range(len(dict_id)):
                Feature_train_num[item, :] = np.mean(Feature_train[target_train[:, 0] == item, :512], axis=0)
            ...
            with tqdm(total=target_test.shape[0], postfix=dict) as pbar:
                for item in range(target_test.shape[0]):
                    ...
                    #                          dict_id_all,
                    #                          4, device)
                    Top, Top_index = get_pre_num(train_csv, Feature_test[item, :], Feature_train_num,
                                                 dict_id, dict_id_species,
                                                 dict_id_all, 5, device)
